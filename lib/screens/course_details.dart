...
class _CourseDetailsState extends State<CourseDetails> {
  ...
  Future getStatData() async {
    return await FirebaseFirestore.instance
        ...
        .get()
        .then((result) {
      attendanceAverage = result.get('attendance average');
      hyperFocus = result.get('hyper focus');
      popQuizAverage = result.get('pop quiz average');
      for (int interval = 0; interval < 24; interval++) {
        chartData2.add(ChartData(x: interval, y: attendanceAverage[interval]));
        chartData3.add(ChartData(x: interval, y: hyperFocus[interval]));
        chartData4.add(ChartData(x: interval, y: popQuizAverage[interval]));
      }
    });
  }

  Future getActiveSession() async {
                        ...
                        Container(
                          ...
                          child: Padding(
                            padding: const EdgeInsets.only(
                                top: 8.0, bottom: 8.0, left: 14.5, right: 14.5),
                            child: Column(
